...
def make_path():
    return "{}_{}_bs{}_fix={}_lr={}".format(opts.expri,opts.savepath, opts.BatchSize, opts.fix_rate, opts.lr, opts.lr_decay_style,opts.lr_decay_ratio)

""" ==================== Models ======================== """

def save_model(model,current = False):
    save_path = make_path()
    ...
    if current:
        model_name = os.path.join(config['checkpoint_base'], save_path, 'current_lr={}.pt'.format(opts.lr))
    else:
